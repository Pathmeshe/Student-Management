...
select Name,count(code) as count_is FROM country where Population>186800 group by Continent having count_is>10 order by count_is;

-- 6)Get the list of courses with num assigned instructor.
SELECT 
    Courses.course_name,
    COUNT(CourseAssignments.instructor_id) AS num_instructors
FROM
    Courses
        INNER JOIN
